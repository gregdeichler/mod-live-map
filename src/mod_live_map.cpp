...
static bool IsBotPlayer(Player* p){
    if(!p) return false;
    // Playerbot detection: session null or no remote address is common but not perfect.
    // Make it configurable via conf: LiveMap.BotDetection = "session_null" etc
    if(!p->GetSession()) return true;
    // Don't rely on empty IP alone - some proxies may have empty, so check bot flag if available
    // If mod-playerbots defines IsPlayerBot, prefer that; fallback to address check
    if(p->GetSession()->GetRemoteAddress().empty()) return true;
    return false;
}

static std::string GetBehaviorState(Player* player, bool isDead, bool inCombat, bool isInFlight)
{
    ...
    // Distinguish real player idle vs bot grinding
    if(player->GetSession() && !player->GetSession()->GetRemoteAddress().empty()) return "Idle";
    return "Grinding";
}
...
public:
    ...
    void OnPlayerUpdate(Player* player, uint32) override {
        ...
        // FIXED throttling - correct m[guidLow] = now
        {
            std::lock_guard<std::mutex> g(_lastUpdateMutex);
            auto& m = isBot ? _lastBotUpdate : _lastPlayerUpdate;
            auto it = m.find(guidLow);
            if(it != m.end() && (now - it->second) < interval) return;
            m[guidLow] = now;
        }
        ...
        std::string behavior = GetBehaviorState(player, isDead, inCombat, isInFlight);
        uint32 totalPlayed = player->GetTotalPlayedTime();
        uint32 levelPlayed = player->GetLevelPlayedTime();
        uint32 money = player->GetMoney();
        float healthPct = player->GetMaxHealth() > 0 ? (float)player->GetHealth()*100.0f / (float)player->GetMaxHealth() : 0;
        float manaPct = player->GetMaxPower(POWER_MANA) > 0 ? (float)player->GetPower(POWER_MANA)*100.0f / (float)player->GetMaxPower(POWER_MANA) : 0;
        uint32 latency = player->GetSession() ? player->GetSession()->GetLatency() : 0;

        sLiveMap->QueueUpdate(guidLow, player->GetName(), mapId, player->GetPositionX(), player->GetPositionY(), player->GetPositionZ(), player->GetZoneId(), player->getLevel(), player->getClass(), isBot, player->GetTeamId(), player->getRace(), guildName, isDead, inCombat, behavior, isInFlight, taxiOrigin, taxiDest, totalPlayed, levelPlayed, money, healthPct, manaPct, latency);
    }
    void OnPlayerLogout(Player* p) override {
        if(!p) return;
        uint32 id=p->GetGUID().GetCounter();
        { std::lock_guard<std::mutex> g(_lastUpdateMutex); _lastPlayerUpdate.erase(id); _lastBotUpdate.erase(id); }
        sLiveMap->RemovePlayer(id);
    }
    void OnPlayerLogin(Player* p) override {
        if(!p) return;
        uint32 id=p->GetGUID().GetCounter();
        std::lock_guard<std::mutex> g(_lastUpdateMutex);
        _lastPlayerUpdate.erase(id); _lastBotUpdate.erase(id);
    }
};
...
public:
    ...
    void OnStartup() override { sLiveMap->Init(); LOG_INFO("module","[LiveMap] Started secure version +Inspector+Behavior+Flight"); }
